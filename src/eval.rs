...
pub fn eval_e<S:Debug>(lctx: &mut HashMap<usize,Value>, pctx: &Program<S>, e: &Expression<S>) -> Value {
   match e {
      Expression::LiteralIntroduction(lps,span) => {
         ...
         match lip {
            ...
            LIPart::InlineVariable(vi) => {
               ...
               for ci in *vs..*ve {
                  lcs.push(vcs[ci]);
               }} else { panic!("v#{} not found at {:?}", vi, span) }
            },
         }}
      ...
      Expression::TupleIntroduction(tps,span) => {
         ...
         match tip {
            ...
            TIPart::InlineVariable(vi) => {
               ...
               }} else { panic!("inline tuple v#{} not found at {:?}", vi, span) }
            },
         }}
      ...
      Expression::VariableReference(li,span) => {
         if let Some(lv) = lctx.get(li) {
            lv.clone()
         } else { panic!("v#{} not found at {:?}", li, span) }
      },
      Expression::FunctionApplication(fi,pxs,span) => {
         if *fi>pctx.functions.len() { panic!("f#{} undefined at {:?}", fi, span); }
         if pxs.len()!=pctx.functions[*fi].args.len() { panic!("f#{} call with wrong arity at {:?}", fi, span); }
         let mut ps = Vec::new();
         for px in pxs.iter() {
      ...
      Expression::Failure(span) => panic!("eval_e(Failure) at {:?}", span),
   }
}

pub fn eval<S:Debug>(p: Program<S>) -> Value {
   let mut top_value = Value::tuple(Vec::new());
